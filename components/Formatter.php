...
class Formatter extends YiiFormatter
{
    ...
    /**
     * Metodo que formata tanto um CPF tanto um CNPJ
     * @param string $string
     * @return string
     */
    public static function asCpfCnpj(string $string): string
    {
        if (empty($string)) {
            return '-';
        }
        ...
        if (strlen($string) <= 11) {
            return static::asCpf($string);
        } else {
            return static::asCnpj($string);
        }
    }
}
